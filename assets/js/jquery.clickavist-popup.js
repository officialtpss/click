(function ($) {
    ...
    $.fn.clickAvist = function(options) {
        /** 
         * Javascript this
         *
         */
         ...
         */
        function init() {
            $(document).on("click.clickAvist", selector, function(e) {
                e.preventDefault();
        ...
        function validateSettings() {
            if (settings.type !== "auto"
                ...
                throw new Error("clickAvist: Type must me \"auto\", \"data\" or \"html\"");
            }

            if (settings.backdrop > 1 || settings.backdrop < 0) {
                throw new Error("clickAvist: Please enter a \"backdrop\" value <= 1 of >= 0");
            }

            if (settings.fadeInDuration < 0 || Number(settings.fadeInDuration) !== settings.fadeInDuration) {
                throw new Error("clickAvist: Please enter a \"fadeInDuration\" number >= 0");
            }

            if (settings.fadeOutDuration < 0 || Number(settings.fadeOutDuration) !== settings.fadeOutDuration) {
                throw new Error("clickAvist: Please enter a \"fadeOutDuration\" number >= 0");
            }
        }
        ...
        function determineType() {
            ...
            if (settings.type === "auto") {
                ...
                throw new Error("clickAvist: could not determine type for \"type: auto\"");
            }
        ...
        function setData() {
            ...
            if (determinedType === "html") {
                if (!settings.htmlSelector) {
                    throw new Error("clickAvist: for \"type: html\" the \"htmlSelector\" option must point to your popup html");
                }

                if (!$(settings.htmlSelector).length) {
                    throw new Error("clickAvist: the \"htmlSelector\": \"" + settings.htmlSelector + "\" was not found");
                }
            ...
            if (determinedType === "data") {
                ...
                if (!data) {
                    throw new Error("clickAvist: for \"type: data\" the \"data-content\" attribute can not be empty");
                }
        ...
        function insertPopupHtml() {
            var content = $("<div/>", {
                "class": "clickavist-popup-content",
                "html": data
            });
            ...
            var intervalId = setInterval(function() {
                if ($("#clickavist-popup").css("opacity") === "1") {
        ...
        function stopPopup() {
            ...
            $("#clickavist-popup").addClass("hide-it");

            // Poll to check if the popup is faded out
            var intervalId = setInterval(function() {
                if ($("#clickavist-popup").css("opacity") === "0") {
                    $("#clickavist-popup").remove();
                    clearInterval(intervalId);
         ...
         * "clickavist-popup" was clicked or "clickavist-popup-content"
         *
         * @param {string} html The html of the popup
         */
        function bindClickPopup(html) {
            $(html).on("click", function(e) {
                if ($(e.target).prop("id") === "clickavist-popup") {
                    stopPopup();
                }
            });
        }
        ...
        function stopBackdrop() {
            var backdrop = $("#clickavist-popup-backdrop");
            ...
            // Poll to check if the popup is faded out
            var intervalId = setInterval(function() {
                if ($("#clickavist-popup-backdrop").css("opacity") === "0") {
                    $("#clickavist-popup-backdrop").remove();
                    clearInterval(intervalId);
                }
            }, 100);
        }
        ...
        function insertBackdropHtml() {
            var content = $("<div/>", {
                "class": "clickavist-popup-backdrop-content"
            });

            var html = $("<div/>", {
                "id": "clickavist-popup-backdrop",
                "class": "hide-it"
            });
